 ...
 * Remark:修改说明<br/>
 */
public class TwoTable4GridGenerator  extends BaseAbstractCodeGenerator{
...
//
	...
	public static void createDir() throws Exception {	
		// 新建目标目录
        (new File(root+"src/com/pdms/data/bo/"+moduleName)).mkdirs();
        (new File(root+"src/com/pdms/data/vo/"+moduleName)).mkdirs();
        (new File(root+"src/com/pdms/action/"+moduleName)).mkdirs();
        (new File(root+"src/com/pdms/service/"+moduleName)).mkdirs();
        (new File(root+"src/com/pdms/service/impl/"+moduleName)).mkdirs();
	...
	public static void createPoVo(String tablename1,String tablename2,String tableid2) throws Exception {
		...
		TwoTable4GridGenerator gg = new TwoTable4GridGenerator();		
	...
	public static void createJava(String tablename1,String tabletitle1,String tablename2,String tabletitle2,String tableid2) throws Exception {
		...
		//生成bs,action,jsp请保证实体已先生成并以导入依赖工程
	 ...
	 * 使用说明
	 * 1)定义子表对象（可以多个）
	 * 例如： EntityTable sub = new EntityTable("PRD_DEV_TROUBLE_DTL(表名)", "TROUBLE_ID(表对应主表外键字段)");
	 *       EntityTable sub2 = new EntityTable("表明", "外键");
	 * 
	 * 2)定义主表Entity（只有一个）
	 * 例如 ：EntityTable mainTable = new EntityTable("PRD_DEV_TROUBLE（主表名称）",sub,sub2(前面定义的EntityTable子表对象));
	 * 
	 * 3)调用gt(主表EntityTable)生成文件
	 */
	public void generatorTable(String tablename1,String tablename2,String id2) throws Exception {
		EntityTable sub = new EntityTable(tablename2, id2);
	 ...
	 * 1)定义子表对象（可以多个）
	 * 例如： 
	 * EntityClazz sub = new EntityClazz(BmCodeName.class,"代码定义","codeType");//参数：子表类Class,实体中文名称，外键属性名
	 * EntityClazz sub2 = new EntityClazz(BmCodeName2.class,"代码定义2","codeType");
	 *
	 * 2)定义主表Entity（只有一个）
	 * 例如 ：
	 * EntityClazz mainClazz = new EntityClazz(BmCodeType.class, "代码类型",sub,sub2);//参数为：类class，中文名称，子表EntityClazz对象,...
	 * 
	 * 3)调用gt(主表EntityClazz)生成文件
	 */
	public void generatorClazzAndJSP(String boclass1,String botitle1,String boclass2,String botitle2,String id2)throws Exception{
		...
		EntityClazz subClazz = new EntityClazz(stcom2.getClass(), botitle2,id2);
		EntityClazz mainClazz = new EntityClazz(stcom1.getClass(), botitle1,subClazz);
		/////////////////////////////////////////////////////////////生成代码
